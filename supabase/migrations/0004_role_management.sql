-- ============================================================
-- 0004_role_management.sql
-- T-CODE-14: role_history (0001_init.sql) already has previous_role_id /
-- new_role_id / actor_id / reason / created_at — confirmed present, no
-- schema change needed there. No change_role-style RPC existed anywhere in
-- 0001-0003 (grepped, confirmed) — this adds the one authorized RPC.
-- Same SECURITY DEFINER + self-checked + audit_logs pattern as
-- set_member_status / moderate_group_member (0001_init.sql).
-- ============================================================
...
begin
  ...
  select rank into v_new_rank from roles where id = p_new_role_id;
